...
(() => {
  ...
  function getReact() {
    ...
    if (!R) throw new Error("runtime: window.React is not available yet");
    return R;
  }
  function getReactDOM() {
    const RD = window.ReactDOM;
    if (!RD) throw new Error("runtime: window.ReactDOM is not available yet");
    return RD;
  }
  ...
  function warnUnresolved(ctx, what) {
    ...
    warnedHoles.add(key);
    console.warn("[app] " + (ctx?.__name || "template") + ": " + what);
  }
  ...
  function createComponentFactory(registry, ensureFetched) {
    ...
    class StreamableComponent extends React.Component {
      ...
      componentDidCatch(e, info) {
        console.error(
          "[app] render error in <" + this.__name + ">:",
          e,
          info?.componentStack || ""
        );
      }
      ...
      render() {
        ...
        if (this.state.__err) {
          return h(
            ...
            h(
              "div",
              { className: "sc-logic-error", "data-ui-chrome": "" },
  ...
  function createExternalModules(onResolved) {
    ...
    function load(kind, url, after) {
      ...
      cache.set(url, null);
      console.info("[app] x-import: loading", url, "(" + kind + ")");
  ...
  function createRuntime(doc = document) {
    ...
    function ensureFetched(name) {
      ...
      (blob ? blob.text() : fetch(target).then((res2) => {
        if (!res2.ok) {
          console.error(
            '[app] sibling fetch for "' + name + '" failed:',
            url,
            "returned",
            res2.status,
        ...
        (e) => console.error(
          '[app] sibling fetch for "' + name + '" threw:',
          url,
          e
        )
      );
    ...
    function updateHtml(name, html) {
      ...
      try {
        ...
        console.error("[app] template compile FAILED for", name, e);
      }
      registry.bump(name);
    }
    function updateJs(name, src) {
      ...
      try {
        ...
        if (r.jsSeq !== seq) return;
        console.error(
          "[app] logic class eval FAILED for",
          name,
  ...
  loadReactUmd().then(init).catch((err) => {
    console.error("[app] failed to load React or boot:", err);
    throw err;
  });
})();
