(() => {
  ...
  const UNSPLASH_HOMEPAGE_HREF =
    'https://unsplash.com/?utm_source=minatsuki&utm_medium=referral';
  ...
  const withReferral = (href) => {
    try {
      ...
      if (!u.searchParams.has('utm_source')) {
        u.searchParams.set('utm_source', 'minatsuki');
      }
  ...
  function flushNow() {
    if (!loaded) return;
    const w = window.__host && window.__host.writeFile;
    if (!w) return;
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      const editable = !!(window.__host && window.__host.writeFile);
